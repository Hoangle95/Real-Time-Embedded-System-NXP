...
static void decoder_ssp0__init(uint32_t SPI_clock_mhz) {
  ...
  for (uint8_t divider = 2; divider <= 254; divider += 2) {
    ...
    LPC_SSP0->CPSR = divider; // Setup PreScale Control[7:0]
  }
}
...
//                                Set up max clock                                      //
//======================================================================================//
static void decoder_max_clock(uint32_t SPI_clock_mhz) { SPI_clock_mhz = SPI_clock_mhz * 1000 * 1000; }
...
static void decoder_ssp0_PINconfig() {
  ...
  GPIO__set_as_output(2, 1); // setup CS- Chip Select (low active)

  gpio__construct_with_function(2, 2, GPIO__FUNCITON_0_IO_PIN);
  GPIO__set_as_output(2, 2); // setup DCS- Data Command Select (low active)
...
static uint8_t decoder_ssp0_transferByte(uint8_t data_transfer) {
  ...
  LPC_SSP0->DR = data_transfer; // Send data, 16-Bits Data Register [15:0]

  // Status Register-BUSY[4] mode
  while (LPC_SSP0->SR & (1 << 4)) {
    ; /* Wait while it is busy(1), else(0) BREAK */
  }
  return (uint8_t)(LPC_SSP0->DR & 0xFF); // READ 16-Bits Data Register [15:0]
}
...
// (P2_1) Chip Select: Transfer SCI command (chose SCI or SDI)
void set_CS_HighActive() { LPC_GPIO2->SET |= (1 << 1); }
void set_CS_LowActive() { LPC_GPIO2->CLR |= (1 << 1); }

// (P2_2) MP3 Data Control: Transfer Data audio
...
void decoder_write_register(uint16_t register_address, uint8_t MSB_byte, uint8_t LSB_byte) {
  while (!get_DREQ_HighActive()) {
    ; // Wait for DREQ go high indicate Start transaction
  }
  set_CS_LowActive(); // Chip Select ON
  ...
  // Write 8 bits word address has been shifted
  decoder_ssp0_transferByte(MSB_byte); // Start Write MSB first
  decoder_ssp0_transferByte(LSB_byte); // End Write LSB last
  while (!get_DREQ_HighActive()) {
    ; // Wait for DREG go high indicating command is complete
  }
  set_CS_HighActive(); // Chip Select OFF
}
...
uint16_t decoder_read_register(uint16_t register_address) {
  ...
  uint16_t finalvalue = 0; // 16 bit empty
...
void decoder_send_mp3Data(uint8_t data) {
  set_XDCS_LowActive();            // Select Transfer data audio
  decoder_ssp0_transferByte(data); // Send SPI Data
  set_XDCS_HighActive();           // Deselect Transfer data audio
}
...
void decoder_setup() {
  ...
  // Deselect --> CS & XDCS (low active)
  set_CS_HighActive();
  set_XDCS_HighActive();

  // Set default Volume
  decoder_write_register(SCI_VOL, 10, 10);
  ...
  decoder_write_register(SCI_CLOCKF, 0x60, 0x00); // Clock frequency 50
